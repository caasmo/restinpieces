...
func route(cfg *config.Config, ap *core.App, cAp *custom.App) {
	...
	ap.Router().Register(
		...
		r.NewRoute(cfg.Endpoints.RequestVerification).WithHandlerFunc(ap.RequestEmailVerificationHandler),
		...
		r.NewRoute(cfg.Endpoints.ConfirmVerification).WithHandlerFunc(ap.ConfirmEmailVerificationHandler),
