...
func route(cfg *config.Config, ap *core.App, cAp *custom.App) {
	...
	ap.Router().Register(
		r.NewRoute(cfg.Endpoints.PublicEndpoints).WithHandlerFunc(ap.PublicEndpointsHandler).WithMiddlewareChain(commonNewMiddleware),
		r.NewRoute(cfg.Endpoints.AllEndpoints).WithHandlerFunc(ap.AllEndpointsHandler).WithMiddleware(ap.JwtValidate).WithMiddlewareChain(commonNewMiddleware),
