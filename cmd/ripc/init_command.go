...
func handleInitCommand(secureStore config.SecureStore, scopeName string) {
	if err := initializeConfig(os.Stdout, secureStore, scopeName); err != nil {
...
func initializeConfig(stdout io.Writer, secureStore config.SecureStore, scopeName string) error {
	if scopeName == "" {
