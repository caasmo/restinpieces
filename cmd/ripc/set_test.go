...
// TestHandleSetCommand_Help verifies that -h prints usage to stdout and
// returns nil instead of an error.
func TestHandleSetCommand_Help(t *testing.T) {
	mockStore := NewMockSetSecureStore(nil)
	var stdout, stderr bytes.Buffer
	ui := UI{Out: &stdout, Err: &stderr}

	err := handleSetCommand(mockStore, []string{"-h"}, ui)
