...
// specifically for serving the compiled asset bundle of Multi-Page Applications
// (MPAs) and Single-Page Applications (SPAs).
//
// It is intended to be registered as the catch-all route ("/") so that all
// requests not claimed by a more specific route fall through to it:
//
...
//	})
//
// For dynamic routes that map to a fixed HTML shell (client-rendered MPA/SPA pages),
// use [MPAShellHandler] instead. It reads the file once at startup, holds it in
// memory, and serves it with zero filesystem activity per request.
//
// For dynamic routes that require data injected into the HTML before sending
// (SSR, no blank-flash, better SEO), implement a custom handler using [ssr.Marshal].
//
...
//     An optional extension generated by your bundler (e.g., CompressExtGzip or CompressExtBrotli).
...
//   - Content-Type Safety: The Content-Type header is always derived from the logical
...
func FSHandler(fsys fs.FS, compressionExt string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 1. Resolve URL to routing instruction
...
func resolveCompression(basePath, compressionExt, acceptEncoding string) (openPath string, encodingName string) {
	...
	case CompressExtGzip:
		encodingName = "gzip"
	case CompressExtBrotli:
		encodingName = "br"
	default:
