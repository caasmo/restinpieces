...
func TestNewConn(t *testing.T) {
	db, err := databasesql.NewConn(newTestDbFile(t))
	if err != nil {
...
func TestNewConn_PragmasOverride(t *testing.T) {
	db, err := databasesql.NewConn(newTestDbFile(t), map[string]string{"busy_timeout": "12345"})
	if err != nil {
		t.Fatalf("failed to open connection: %v", err)
	}
	t.Cleanup(func() {
...
func TestNewConn_MissingFile(t *testing.T) {
	...
	_, err := databasesql.NewConn(dbPath)
	if err == nil {
