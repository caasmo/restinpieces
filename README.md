...
### Cache
For in-memory caching, the framework ships its own preallocated LRU cache ([`cache/restinpieces`](https://pkg.go.dev/github.com/caasmo/restinpieces/cache/restinpieces)), built entirely on the Go standard library. The `cache.Cache` interface lets you swap in your own implementation via `WithCache`; a [ristretto](https://github.com/dgraph-io/ristretto)-based implementation is available at [restinpieces-cache](https://github.com/caasmo/restinpieces-cache).
