...
- preRouter handling -> we allow disable in config, we do not allow adding, can put in normal middleware.
    ...
    The Simplest Path: No User Pre-Router Customization:
        • Framework: Provides its internal chain (BlockIP, RateLimit, etc.).
        Each component is enabled/disabled via config. The framework builds
        only
              this chain based on the config.
                  • User: Configures which internal middleware pieces are
                  active. They cannot inject their own code into the pre-router
                  sequence. If they need
                        custom logic globally, they must apply it via common
                        route middleware (accepting it runs after routing).
